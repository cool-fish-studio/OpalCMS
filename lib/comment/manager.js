...
//查找全部
module.exports.findAllPaginated = function (contentID, queryOptions, callback)
{
    //此类数据 返回格式包含列表和数量 用来处理后期分页问题
    async.parallel({
        total: Comment.countAll.bind(Comment, contentID),
        comments: Comment.findAll.bind(Comment, contentID, queryOptions)
    }, function (error, args)
    {
        if (error) return callback(error, null);
        callback(
